...
class ModelManager:
    ...
    def __init__(
        ...
        if vision_base is None:
            vision_base = os.environ.get("VISION_API_BASE")

        if language_model_name is None:
            language_model_name = os.environ.get(
                "LANGUAGE_MODEL", "gpt-oss-120b"
            )
        ...
        if language_api_key is None:
            language_api_key = os.environ.get("GROQ_API")
        if vision_api_key is None:
            vision_api_key = os.environ.get("huggingface")
