...
class Agent:
    ...
    #---------------------------------------------------------------------------------------
    async def _dynamic_wait(self, error_idx: int, tokens_used: int, tokens_requested: int):
        """
        Smart backoff based on rate limits.
        - Adjusts wait time according to token usage and retry attempt.
         - Adds random jitter to prevent synchronized retries.
        """
        assert error_idx >= 0, "error_idx must be non-negative"

        TOKEN_LIMIT = 8000
        BASE_WAIT = 2  
        MAX_WAIT = 60  
        ratio = min((tokens_used + tokens_requested) / TOKEN_LIMIT, 1.0)

        wait_time = BASE_WAIT * (2 ** error_idx) * (1 + ratio)
        ...
        self.logger.warning(
            f"[RateLimit] Wait {wait_time:.1f}s (retry {error_idx}, tokens_used={tokens_used}, requested={tokens_requested})"
        )
    ...
    async def retry(
        ...
        **client_kwargs,
        ...
        tokens_requested = getattr(e, "tokens_requested", 0)
        await self._dynamic_wait(error_idx, tokens_used, tokens_requested)
