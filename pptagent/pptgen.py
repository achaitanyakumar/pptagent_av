...
class PPTGen(ABC):
    ...
    def set_reference(
        ...
        return self
    @traceable(name="Generate Presentation")
    ...
    async def generate_outline(
        ...
        if hasattr(source_doc, "iter_medias"):
          mineru_images = [m.path for m in source_doc.iter_medias()]
          image_iter = iter(mineru_images)
          for item in outline:
             new_images = []
             for _ in item.images:
                 try:
                    new_images.append(next(image_iter))
                 except StopIteration:
                    break
             item.images = new_images
        return self._add_functional_layouts(outline)
    
    @traceable(name="Generate Slide")
    @abstractmethod
    def generate_slide(
            ...
            for father, pic in pictures:
               if pic.area / pic.slide_area < area_ratio:
                  ...
                  normalized_caption = re.sub(
                    r'^(Background|Decorative|Shape|Image):?\s*',
                    '',
                    pic.caption or '',
                    flags=re.I
                ).strip()
                  ...
                  try:
                      layout.remove_item(normalized_caption)
                  except ValueError:
                     logger.warning(
                        f"Skipping unmatched caption during hide_small_pics: {pic.caption}"
                    )
...
class PPTAgent(PPTGen):
    ...
    async def _generate_content(
        ...
        """
        with trace(name="Content Generation", inputs={"slide_desc": slide_description}) as run:
    ...
    #--------------------------------------------------------------
    @traceable(name="Generate edit_slide")
    async def _edit_slide(
        ...
        #--------------------------------------------------------------------------------------
    @traceable(name="Validate Content")
    ...
    @traceable(name="Generate Commands")
