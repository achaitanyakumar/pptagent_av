...
class LLM:
    ...
    async def test_connection(self) -> bool:
        ...
        """
        try:
          if "huggingface" in (self.api_base or ""):
            # minimal inference call
             ...
             if any(self.model in mid or mid.endswith(self.model) for mid in model_ids):
                 return True
             else:
                logging.warning(f"Model {self.model} not found in available models: {model_ids}")
                return False
    ...
    async def __call__(
        ...
        **client_kwargs,
        ...
        Returns:
            Union[str, Dict, List, Tuple]: The response from the model.
        """
        if "gpt-oss" in self.model.lower() or "qwen3" in self.model.lower():
              client_kwargs.update(think_mode.client_kwargs)
